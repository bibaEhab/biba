...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b w-full bg-[rgba(255,255,255,0.04)]", className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => {
  // Determine icon based on open/closed state
  ...
  return (
    <AccordionPrimitive.Header className="flex w-full">
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(
          "flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all hover:underline text-left [&[data-state=open]>svg]:rotate-180 w-full",
          className,
        )}
        {...props}
      >
